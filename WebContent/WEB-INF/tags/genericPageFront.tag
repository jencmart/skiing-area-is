    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link  <c:if test="${viewname=='products'}"> active</c:if>" href="/products">Produkty</a>
                ...
                <c:choose>
                    <c:when test="${authenticated}">
                        ...
                        <c:otherwise>
                            <li class="nav-item  <c:if test="${viewname=='profile'}"> active</c:if>">
                                <a class="nav-link" href="/profile"> <i class=" fa fa-user "    <c:if test="${viewname!='profile'}"> style="color:grey" </c:if>     <c:if test="${viewname=='profile'}"> style="color:white" </c:if>></i>
                                    Profil</a>
                            </li>
                        ...
                        <li class="nav-item  <c:if test="${viewname=='login'}"> active</c:if>">
                            <form action = "/logout" method = "post">
                                <button class="btn btn-danger btn-sm" type="submit"> <i class="fa fa-power-off"></i> Odhlasit se</button>
                                <sec:csrfInput/>
                            </form>
                        </li>
                    ...
                    <c:otherwise>
                        <li class="nav-item  <c:if test="${viewname=='profile'}"> active</c:if>">
                            <a class="nav-link" href="/profile"> <i class=" fa fa-user "    <c:if test="${viewname!='profile'}"> style="color:grey" </c:if>     <c:if test="${viewname=='profile'}"> style="color:white" </c:if>></i>Profil</a>
                        </li>
                    </c:otherwise>
                </c:choose>
                ...
                <li class="nav-item  <c:if test="${viewname=='shoppingcart'}"> active</c:if> " >
                    <a class="nav-link" href="/shoppingcart">
                        ...
                        <i class="fas fa-shopping-basket "  <c:if test="${viewname!='cart'}"> style="color:grey" </c:if>     <c:if test="${viewname=='cart'}"> style="color:white" </c:if>></i>
                        <span id="shoppingCartCount" class="fa-layers-counter fa-2x" style="background:Tomato" > ${shoppingCartCount} </span>
                        </span>
                    </a>
                </li>
            </ul>
